...
def new_file():
    ...
    # Update status bars
    root.title('New File - Simple text editor!')
...
def open_file():
    ...
    # Grab Filename
    text_file = filedialog.askopenfilename(initialdir="C:/Documents/", title="Open File", filetypes=(
    ...
    name = name.split("/")
    # name.replace("C:/", "")
    root.title(f'{name[-1]} - Simple text editor!')
...
# Save As File
def save_as_file():
    text_file = filedialog.asksaveasfilename(defaultextension=".*", initialdir="C:/Documents/", title="Save File",
...
edit_menu.add_command(label="Redo", command=my_text.edit_undo, accelerator="(Ctrl+y)")
