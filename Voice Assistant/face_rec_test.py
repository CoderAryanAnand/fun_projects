...
class DlibFaceUnlock:
    ...
    def __init__(self):
        ...
        # this is compare the new label ids to the old label ids dictionary seeing if their has been any new users or
        # old users being added to the system, if there is no change then nothing will happen
        self.og_labels = 0
        if self.labels_ids != self.og_labels:
            ...
            for self.i in self.labels_ids:
                ...
                no_of_imgs = len([filename for filename in os.listdir('images/' + self.i)
                                  if os.path.isfile(os.path.join('images/' + self.i, filename))])
                print(no_of_imgs)
                for imgNo in range(1, (no_of_imgs + 1)):
    ...
    def ID(self):
        ...
        # setting the running variable to be true to allow me to known if the face recog is running
        self.running = True
        self.face_names = []
...
def register():
    ...
    number_of_file = len([filename for filename in os.listdir('images/' + name.get())
    ...
    while True:
        ...
        if k % 256 == 27:
            ...
            # SPACE pressed
            img_name = str(number_of_file) + ".png"
            cv2.imwrite(img_name, frame)
            print("{} written!".format(img_name))
            os.replace(str(number_of_file) + ".png",
                       "images/" + name.get().lower() + "/" + str(number_of_file) + ".png")
            cam.release()
            cv2.destroyAllWindows()
            break
...
def login():
    ...
    log_book = open("log_book.txt", "a")
    log_book.write(datetime.datetime.now().strftime('%d-%m-%Y || %H:%M:%S __%A__\n'))
    log_book.close()
    raise_frame(userMenuFrame)
    root.destroy()
...
def reg_frame_raise_frame():
    raise_frame(regFrame)


def log_frame_raise_frame():
    raise_frame(loginFrame)
...
def main():
    loadfont('Pacifico-Regular.ttf', private=False, enumerable=True)
    loadfont('Oswald-VariableFont_wght.ttf', private=False, enumerable=True)
    ...
    tk.Label(loginFrame, text="Face Recognition", font=(loadfont('Pacifico-Regular.ttf', private=False, enumerable=True),
                                                        60), bg="white").grid(row=1, column=1,
                                                                              columnspan=5)
    loginButton = tk.Button(loginFrame, text="Login", bg="white", font=(loadfont('Oswald-VariableFont_wght.ttf',
                                                                                 private=False, enumerable=True), 30),
                            command=login)
    loginButton.grid(row=2, column=3)
    # regButton = tk.Button(loginFrame, text="Register", command=reg_frame_raise_frame, bg="white", font=("Arial", 30))
    ...
    # tk.Button(userMenuFrame, text="Back", font=("Arial", 30), command=log_frame_raise_frame).grid(row=2, column=1)
