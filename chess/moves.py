...
def end_loc(move_desc):
    if len(move_desc) == 3 and not '=' in move_desc:
        endloc = move_desc[-2:]
    elif '=' in move_desc:
        endloc = move_desc[:2]
    elif len(move_desc) == 2:
        endloc = move_desc
    elif len(move_desc) == 4 and not 'x' and '+' in move_desc:
        endloc = move_desc[-2:]
    elif len(move_desc) == 4 and 'x' and not '+' and not '=' in move_desc:
        endloc = move_desc[-2:]
    elif len(move_desc) == 4 and '+' and not '+' in move_desc:
        endloc = move_desc[-3:-1]
    elif '=' and 'x' in move_desc:
        endloc = move_desc[2:4]
    else:
        endloc = None
...
if __name__ == '__main__':
    ...
    for position, string in enumerate(split_list):
        if len(string) != 0:
            if string[-1] == ' ':
                split_list[position] = string[:-1]
        else:
            del split_list[0]
    ...
    for move_no, move in enumerate(split_list):
        ...
        w_piece = getmove(split_moves[0])
        b_piece = getmove(split_moves[1])
        w_endloc = end_loc(split_moves[0])
        b_endloc = end_loc(split_moves[1])
        print(split_moves)
        print(w_endloc, b_endloc)
        print(w_piece, b_piece)
        chess.svg.board(board=b)
        new_dict[move_no + 1] = {
            'White': {'MoveDesc': split_moves[0], 'Piece': w_piece, 'EndLoc': w_endloc},
            'Black': {'MoveDesc': split_moves[1], 'Piece': b_piece, 'EndLoc': b_endloc}
        }
    print(new_dict)
